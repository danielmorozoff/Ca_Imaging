...
% gather data
fid = fopen(fn);
data = textscan(fid, '%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d','delimiter',',','BufSize',4096*16,'Commentstyle','trial');
fclose(fid);
...
for trial = 1:numtrials
    ...
    for licknum = 1:length(lickstarts)
        ...
        y = 3*trial-2;
        wth = 1; %double(data{1}(lickidx(lickends(licknum))) - data{1}(lickidx(lickstarts(licknum))));
    ...
    y = 3*trial;
    w = max(1,double(data{1}(maxtidx)-data{1}(mintidx)));
    h = 1;
