...
%% function that updates the image display
    ...
    function [] = updateimage()
        ...
        if useonsetonly
            newmap = onsetmap(:,logical(trialstate),startframe:endframe);
        else            
            newmap = snmap(:,logical(trialstate),startframe:endframe);
        end
        ...
        validcells = numresponses>=((pctcutoff/100)*sum(trialstate)); 
        ...
        for q=1:length(celllist)
            ...
            overlayimg(ymin:ymax,xmin:xmax) = max(overlayimg(ymin:ymax,xmin:xmax),numresponses(cellnumber)*(cellspatialfilters{cellnumber}>0));
            cellidmap(ymin:ymax,xmin:xmax) = cellnumber*(cellspatialfilters{cellnumber}>0);
        end
        hold on
        DataImg = imshow(imgdata,[],'Parent',ImageAxes);        
        customcmap = [[0 0 0];jet(sum(trialstate))];
